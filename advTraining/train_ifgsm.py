...
X_train_new = np.load("new_train.npy").astype(np.float32)
y_train_new = y_train
...
def train(model, init_epoch = 0):
    hist = model.fit(X_train, y_train, initial_epoch = init_epoch, epochs=10, shuffle=True, validation_data=(X_test, y_test))
...
if os.path.exists(Model_Name):
    #train_ifgsm,读取model_cifar的权重
    model.load_weights('model_cifar.h5')
